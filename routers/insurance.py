...
def update_insurance(insurance_id: int, data: InsuranceCreate, db: Session = Depends(get_db)):
    ...
    insurance.charges = predict_charges(data.smoker, data.age, data.bmi)
...
def delete_insurance(insurance_id: int, db: Session = Depends(get_db)):
    ...
    return {"detail": "Seguro eliminado exitosamente"}